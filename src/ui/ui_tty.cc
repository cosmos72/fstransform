...
ft_ui_tty::ft_tty_window::ft_tty_window()
    : len(0), h0(0), h(0)
{ }
...
int ft_ui_tty::init(const char * tty_name)
{
#ifdef TIOCGWINSZ
    struct winsize wsz;
    int err = 0, fd = -1;
...
int ft_ui_tty::start(FT_IO_NS ft_io * io)
{
...
void ft_ui_tty::show_io_read(ft_from from, ft_uoff offset, ft_uoff length)
{
    const ft_tty_window & window = from == FC_FROM_DEV ? this_dev : this_storage;

    show_io_op(false, window, offset, length);
}

void ft_ui_tty::show_io_write(ft_to to, ft_uoff offset, ft_uoff length)
{
    const ft_tty_window & window = to == FC_TO_DEV ? this_dev : this_storage;

    show_io_op(true, window, offset, length);
}

void ft_ui_tty::show_io_copy(ft_dir dir, ft_uoff from_physical, ft_uoff to_physical, ft_uoff length)
{
    show_io_read(ff_from(dir), from_physical, length);
    show_io_write(ff_to(dir), to_physical, length);
}

void ft_ui_tty::show_io_op(bool is_write, const ft_tty_window & window, ft_uoff offset, ft_uoff length)
{
    if (need_clr) {
        need_clr = false;
