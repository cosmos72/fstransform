...
#include "work.hh"              // for ft_work<T>::show()
...
int ff_zero_loop_file_holes(int argc, char ** argv)
{
    ...
    do {
        ...
        ft_vector<ft_uoff> loop_extents;
        ...
        ft_map<ft_uoff> loop_holes_map;
        loop_holes_map.complement0_logical_shift(loop_extents, eff_block_size_log2, dev_len);
        ft_work<ft_uoff>::show("loop-holes", "", eff_block_size_log2, loop_holes_map, FC_INFO);

        ft_map<ft_uoff>::const_iterator iter = loop_holes_map.begin(), end = loop_holes_map.end();
        ...
        for (; iter != end; ++iter) {
            const ft_map<ft_uoff>::value_type & extent = *iter;
