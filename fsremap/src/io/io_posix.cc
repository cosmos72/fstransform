        ...
        for (i = 0; i < FC_FILE_COUNT; i++) {
            ...
            if (i == FC_DEVICE) {
                ...
                if ((err = ff_posix_size(fd[i], & len[i])) != 0) {
                    ...
                    err = ff_log(FC_ERROR, 0, "error: %s size = %"FT_ULL" bytes exceeds %s length = %"FT_ULL" bytes",
 ...
 */
int fr_io_posix::copy_bytes(fr_dir dir, fr_vector<ft_uoff> & request_vec)
{
    int err = 0;
...
int fr_io_posix::copy_bytes(fr_dir_posix dir, const fr_extent<ft_uoff> & request)
{
