					  ...
					  FT_IO_NS fr_io & io)
{
    if (fr_work<ft_uint>::check(io) == 0) {
        ...
        return fr_work<ft_uint>::main(loop_file_extents, free_space_extents, to_zero_extents, io);
    }
    ff_log(FC_INFO, 0, "using full (%" FT_ULL " bit) remapping algorithm", (ft_ull)8*sizeof(ft_uoff));
    return fr_work<ft_uoff>::main(loop_file_extents, free_space_extents, to_zero_extents, io);
}
